...
export function TractorsManager() {
  ...
  const resetForm = () => { setNumber(""); setNotes(""); setIsActive(true); };
  const openCreate = () => { resetForm(); setCreateOpen(true); };
  const openEdit = (t: Tractor) => {
    setEditId(t.id); setNumber(t.number); setNotes(t.notes ?? ""); setIsActive(t.isActive);
  };
  const closeEdit = () => setEditId(null);

  const createMutation = useMutation({
    mutationFn: async () =>
      await apiRequest("POST", "/api/tractors", { number: number.trim(), notes: notes.trim() || null, isActive }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      return await apiRequest("PUT", `/api/tractors/${editId}`, { number: number.trim(), notes: notes.trim() || null, isActive });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: TRACTORS_KEY });
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Configured Tractors / Box Trucks</CardTitle>
