...
export function TrailersManager() {
  ...
  const createMutation = useMutation({
    mutationFn: async () =>
      await apiRequest("POST", "/api/trailers", { number: number.trim(), notes: notes.trim() || null, isActive }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      return await apiRequest("PUT", `/api/trailers/${editId}`, { number: number.trim(), notes: notes.trim() || null, isActive });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: TRAILERS_KEY });
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Configured Trailers</CardTitle>
