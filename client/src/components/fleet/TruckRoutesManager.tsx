...
export function TruckRoutesManager() {
  ...
  const resetForm = () => { setName(""); setDescription(""); setIsActive(true); };
  const openCreate = () => { resetForm(); setCreateOpen(true); };
  const openEdit = (r: TruckRoute) => {
    setEditId(r.id); setName(r.name); setDescription(r.description ?? ""); setIsActive(r.isActive);
  };
  const closeEdit = () => setEditId(null);

  const createMutation = useMutation({
    mutationFn: async () =>
      await apiRequest("POST", "/api/truck-routes", { name: name.trim(), description: description.trim() || null, isActive }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      return await apiRequest("PUT", `/api/truck-routes/${editId}`, { name: name.trim(), description: description.trim() || null, isActive });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ROUTES_KEY });
  ...
  const addStop = (locationId: number) => { if (!stopIds.includes(locationId)) updateStops([...stopIds, locationId]); };
  const removeStop = (locationId: number) => updateStops(stopIds.filter(id => id !== locationId));
