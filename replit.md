...
- **Leadership Scheduling Constraints**: Three-pass approach for manager coverage. Team leads can only open if a higher-tier manager (store manager or assistant manager) closes that day, and vice versa. Higher-tier managers are prioritized for days where team leads already have shifts (from fixed schedules or templates). Random off days for higher-tier managers are protected on team-lead-dependent days.
