...
- **Trailer Manifest**: Live trailer load tracking with atomic +/- item counts, photos, status workflow, and manifest history. Manifests can be created from the Daily Route page via a side-drawer Sheet (replacing the old Dialog) — one per (routeId, serviceDate), enforced by a partial unique index — pre-filling From/To/equipment quantities. The drawer includes warehouse, trailer, and driver dropdowns; after creation the user stays on the Daily Route page with a toast linking to the new manifest. Route badges show inline manifest status (green chip with status label and trailer number) when a manifest already exists for that route/date. GET `/api/daily-route/manifests?date=` returns existing manifests for badge rendering.
