...
- **Order Form**: Migrated from WP Forms, this feature allows managers to submit equipment orders to an external MySQL database (`/orders/new`). Supports 4 order types with conditional field visibility: Transfer and Receive (equipment requested/returned, category gaylords, seasonal items), End of Day/Equipment Count (full/empty counts, outlet items, rotated items for Outlet locations, seasonal returns, eCom containers), Donors (donor count), and Supplemental Production (apparel/wares production for Central Processing/Lee Harvard). Submission history is viewable at `/orders` with filtering by date range, location, and order type. Email notifications are sent to configured recipients on each submission (uses the same Outlook/Graph API sender as HR notifications). Notification recipients are configured in Settings under "Order Submission Notifications" (`orderNotificationEmails` in `global_settings`). Routes in `server/routes/orders.ts`, MySQL connection in `server/mysql.ts`. Requires secrets: MYSQL_HOST, MYSQL_PORT, MYSQL_DATABASE, MYSQL_USER, MYSQL_PASSWORD.
