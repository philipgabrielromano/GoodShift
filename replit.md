...
- **Trailer Manifest**: Live trailer load tracking with atomic +/- item counts, photos, status workflow, and manifest history. Manifests can also be generated directly from a Daily Route — one per (routeId, serviceDate), enforced by a partial unique index — pre-filling From/To/equipment quantities and leaving trailer# and driver blank for the operator.
