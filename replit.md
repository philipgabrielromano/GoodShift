...
- **Production Station Limits**: Configurable per-location limits for Apparel Processor and Wares/Shoes Pricing stations. Both the AI and non-AI schedulers respect these limits when generating schedules, and the validator warns when daily station limits are exceeded. A value of 0 means unlimited.
- **Production Worker Scheduling Strategy**: Full-time production workers (apparel processors and donation/wares pricers) are scheduled first on full morning shifts (8-4:30). Part-timers fill remaining station seats with full shifts. Part-timers who cannot work full shifts get afternoon shifts (4:30-8:30 PM) to extend station coverage after full-timers leave. No short shifts for production workers during morning scheduling.
