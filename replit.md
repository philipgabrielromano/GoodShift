...
- **Production Worker Scheduling Strategy**: Uses a two-phase approach: (1) First fills ALL station slots every day up to the configured station limit for both apparel processors and donation pricers. (2) Then adds extra production shifts beyond station limits on Friday and Saturday only if labor is available. Full-time production workers get morning shifts first; part-timers fill remaining stations with full shifts or afternoon shifts (4:30-8:30 PM) to extend coverage.
...
- **Active/Inactive Employee Toggle**: On Occurrence Report and Attendance pages, managers and admins can switch between viewing active and inactive employees.
