...
- **Order Form**: Equipment order submission to external MySQL, with submission history, filtering, and email notifications. Warehouse review (`/order-submissions`) supports one-click "Approve as requested" or "Adjust & approve" — the latter lets transportation overwrite any of the 17 adjustable `*_requested` quantities (equipment, raw category gaylords, seasonal saved-stock) before approving. Originals plus optional reason are snapshotted to the audit log; downstream inventory exports read the new values directly. Approval is wrapped in `SELECT ... FOR UPDATE` + status guard with a 0-row 409 conflict path; seasonal balance is re-validated against the *adjusted* values, not the originals; non-allowlisted columns are rejected with 400. Approval email lists "(requested N)" callouts whenever any line was changed. **Store-scoped access control**: anyone WITHOUT `orders.approve` (regular store / ordering staff) can only see, submit, edit, receive, or delete orders for their own assigned `locationIds` — enforced server-side on every order endpoint via `getUserAllowedLocationNames(user)` (returns `null` for admins/approvers = unrestricted, `string[]` for store users, `[]` if unassigned = sees nothing). Cross-store reads return 404 (not 403) so existence isn't leakable. UI dropdowns on OrderForm and OrderSubmissions are filtered to match. Aggregate seasonal-balances view also respects the same scope so a store user with `seasonal_inventory.view` can't read other stores' deposits.
- **Trailer Manifest**: Live trailer load tracking with atomic +/- item counts, photos, status workflow, and manifest history. Manifests can be created from the Daily Route page via a side-drawer Sheet — route selection is optional. The drawer shows all ordered items for the date with checkboxes and editable quantities; the dispatcher picks which items go on the manifest. Selecting a specific route filters items to that route and auto-fills the destination; selecting "All routes" shows everything and requires a manual destination. Includes warehouse, trailer, and driver dropdowns. After creation the user stays on the Daily Route page with a toast linking to the new manifest. Route badges show inline manifest status (green chip with status label and trailer number) when a manifest already exists for that route/date. One manifest per (routeId, serviceDate) enforced by partial unique index when a route is selected; route-less manifests have no duplicate constraint. GET `/api/daily-route/manifests?date=` returns existing manifests for badge rendering.
...
- **Schedule Audit Report** (`/reports/schedule-audit`): Shows scheduled hours per job code as a percentage of total scheduled hours for a selected store and week. CSS-based horizontal stacked bars give an at-a-glance visual of labor allocation across roles. Supports multi-week selection (1/2/4/8/13 weeks) with automatic averaging. Detail table below shows hours and percentages per job code per week. API at `GET /api/reports/schedule-audit?weekStarts=...&location=...` joins `shifts` with `employees` (using `crossTrainedRole` or `jobTitle`), aggregates by 7-day windows, and enforces location-based access control. Permission: `reports.schedule_audit` (admin, manager). Dates validated server-side as YYYY-MM-DD; non-admin users with no assigned locations see zero data.
