...
- **Production Worker Scheduling Strategy**: Uses a two-phase approach: (1) First fills ALL station slots every day up to the configured station limit for both apparel processors and donation pricers. (2) Then adds extra production shifts beyond station limits on Friday and Saturday only if labor is available. Workers are sorted by fewest days scheduled so far (equal-distribution sort) before each day's slot-filling, ensuring apparel processors and donation pricers are spread evenly across all 7 days rather than front-loaded. Afternoon part-timer shifts (4:30-8:30 PM) extend coverage after morning full-timers leave.
